...
exports.updatePaymentStatus = onRequest(async (req, res) => {
    try {
        ...
        logger.error(`Error updatePaymentStatus: ${e}`)
        res.status(400).send()
    }
})
...
exports.updateRefundStatus = onRequest(async (req, res) => {
    try {
        ...
        const dataSnap = await orderRef.get()
        const orderStatus = dataSnap.data().status

        if (orderStatus == "refunded" || orderStatus == "refund failed") {
            res.status(200).end()
        }
        ...
        logger.error(`Error updateRefundStatus: ${e}`)
        res.status(400).send()
    }
})
...
exports.placeDeliveryOrder = onDocumentUpdated("/orders/{docId}", async (event) => {
    try {
        ...
        logger.log(`deliveryId ${deliveryId}`)

        if (data.status != 'paid' || deliveryId) {
            return;
        }
        ...
        const [addressSnap, userSnap, orderItemsSnap] = await Promise.all([
            ...
            firestore.collection('orders').doc("a69b60c3-a5ec-44b6-a96c-d3b450edb400").collection("orderItems").get(),
        ])
        ...
        logger.error(`Error placeDeliveryOrder: ${e}`)
    }
});